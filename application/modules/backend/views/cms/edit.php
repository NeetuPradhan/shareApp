...
<div class="box box-primary">
    ...
    <div class="box-body">
        ...
        <form role="form" method="post" action="<?=base_url().'backend/cms/edit/'.$info['id']?>" enctype="multipart/form-data">
        ...
        <div class="col-lg-6">
            <div class="form-group <?php if(form_error('title')) echo 'has-error' ?>">
                ...
                <input name="title" type='text' class="form-control" placeholder="Enter Page Title" value="<?=set_value('title',$info['title']);?>">
                <?=form_error('title')?>
            </div>
            <div class="form-group <?php if(form_error('head_text')) echo 'has-error' ?>">
                <label>Heading</label>
                <input name="head_text" placeholder="Enter Page Heading" value="<?=set_value('head_text',$info['head_text']);?>" class="form-control">
                <?=form_error('head_text')?>
            </div>
            <div class="form-group <?php if(form_error('meta_keywords')) echo 'has-error' ?>">
                <label>Meta Keywords</label>
                <textarea name="meta_keywords" placeholder="Enter meta keywords" class="form-control"><?=set_value('meta_keywords',$info['meta_keywords']);?></textarea>
                <?=form_error('meta_keywords')?>
            </div>
            <div class="form-group <?php if(form_error('meta_description')) echo 'has-error' ?>">
                <label>Meta Description</label>
                <textarea name="meta_description" placeholder="Enter meta description" class="form-control"><?=set_value('meta_description',$info['meta_description']);?></textarea>
