...
class Member extends MX_Controller {
	...
	public function add() {
		...
		if($this->form_validation->run()==TRUE) {
            ...
            $inserted_id = $this->member_model->register($activation_code);
   			if($activation_code!='system_error') {
			    if($this->member_model->reg_confirmation_email($activation_code)) {
		        	...
		        	$this->member_model->hard_delete_user($inserted_id);
	...
	function activation_process($key='', $hash_email='') {
		$email = $this->member_model->activated($key, $hash_email);
        if($email) {
        	$this->session->set_userdata( 'user_flash_msg_type', "success" );
